...
function update(dt){
  ...
  // 지상 구간(0~110, zoneName의 "지상" 기준과 정확히 일치): 캐릭터가 올라갈수록 부드럽게 줌아웃해서
  // 캐릭터+땅이 항상 같이 보이도록, 필요한 줌 값을 매 고도마다 직접 역산해서 사용
  // 상공 구간(170~): 캐릭터/줌이 상공 모드로 안정됨 (아이템/기믹 활용 가능하도록)
  // 그 사이(110~170): 자연스럽게 전환

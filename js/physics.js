...
function trySlam(){
  ...
  slamCount++;
  isSlamming = true;
  // 순수 수직 강하: 아래로 내려찍는 동작이므로 수평 속도는 건드리지 않음
  vy = -Math.max(65, Math.abs(vy)*1.6 + 40);
  screenShake(4, 0.15);
...
function applyItemEffect(type){
  ...
  else if (type === "mango"){ vx = Math.max(2, vx - MANGO_SPEED_PENALTY); spinTimer = 0.9; showToast("망고... 속도 -"+MANGO_SPEED_PENALTY); playSfx('mango'); }
  else if (type === "star"){
    starTimer = 5;
